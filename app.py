...
if uploaded_file is not None:
    ...
    # Deteksi otomatis nama kolom tanggal jika berbeda di file Excel user
    col_mr_date = next((col for col in df.columns if 'mr' in col.lower() and 'date' in col.lower() or 'tanggal' in col.lower() and 'm' in col.lower()), 'MR Date')
    col_tgl_penyerahan = next((col for col in df.columns if 'penyerahan' in col.lower() or 'tgl' in col.lower() and 'serah' in col.lower()), 'Tgl Penyerahan')

    # Jika nama kolom tidak persis sama, gunakan pencarian kolom bertipe tanggal/string waktu
    if col_mr_date not in df.columns and len(df.columns) > 2:
        col_mr_date = df.columns[1] 
    if col_tgl_penyerahan not in df.columns and len(df.columns) > 3:
        col_tgl_penyerahan = df.columns[3] 

    # Hitung Jarak Waktu (Lead Time) otomatis
    if col_mr_date in df.columns and col_tgl_penyerahan in df.columns:
        df[col_mr_date] = pd.to_datetime(df[col_mr_date], errors='coerce')
        df[col_tgl_penyerahan] = pd.to_datetime(df[col_tgl_penyerahan], errors='coerce')
        df['Lead_Time'] = (df[col_tgl_penyerahan] - df[col_mr_date]).dt.days
    else:
        df['Lead_Time'] = 0

    # LOGIKA KATEGORI YANG AMAN DARI ERROR KEYERROR
